...
local function forEachSpot(POIModel) -- Function that is ran for each boombox to work
	...
	if not (BF and CO and Dad and ARO) then
		warn(("Spot doesn't meet the requirements! (%s)"):format(POIModel:GetFullName()))
		return
	end
	...
	-- Spot stuff
	local TakenFunction = function(plr:any, spot:any)
		local char = plr.Character
		local HRP = char:FindFirstChild("HumanoidRootPart")

		HRP.Anchored = true
		HRP.CFrame = spot.Part.CFrame
		...
		-- Send the (Boombox, Spot, Player, isOwner)
		Event:Fire("SpotJoin",POI.Model,spot.Part,plr,POI.Ownership == plr)
	end
	local ExtraTakenFunction = function(plr:any, spot:any)
		local char = plr.Character
		local HRP = char:FindFirstChild("HumanoidRootPart")
		...
		-- Send the (Boombox, Spot, Player, isOwner)
		Event:Fire("SpotJoin",POI.Model,spot.Part,plr,false)
	end
	--[[
	...
	--]]
	POI.BFSpot.Taken:Connect(TakenFunction)
	POI.DadSpot.Taken:Connect(TakenFunction)
	POI.BFSpot2.Taken:Connect(ExtraTakenFunction)
	POI.DadSpot2.Taken:Connect(ExtraTakenFunction)
	...
	SpotFunction.OnInvoke = function(msgType,...)
		...
		elseif msgType == "InitializeCompRemote" then -- Starts the current spot gameplay.
			local msgSpot,whoStart,songModule,songDisplay,songSettings = ...
			--print(...)
			...
			local extraSpeed = 1 -- A variable that controls how fast or slow the song should go
			local instVolume = 2
			local voiceVolume = 2
			--if not SongIds[songDisplay] then return end
			if SongIds[songDisplay] then
				if SongIds[songDisplay].PlaybackSpeed then
					extraSpeed = SongIds[songDisplay].PlaybackSpeed
				end
				
				instVolume = (SongIds[songDisplay].InstrumentalVolume) or 2
				voiceVolume = (SongIds[songDisplay].VoicesVolume) or 2
			end
			
			if instVolume then
				SpotSound.Volume = 1 * (instVolume/2)
			end
			if voiceVolume then
				SpotSound.Volume = 1 * (voiceVolume/2)
			end
			
			-- Modify the playback rate of the stages
			SpotSound.PlaybackSpeed = extraSpeed * songSettings.SpeedModifier
			SpotVocals.PlaybackSpeed = extraSpeed * songSettings.SpeedModifier
			...
			local thread_Round = function()
				...
				compRemote.OnServerEvent:Connect(function(plr,msg,...) -- COMPERTITION REMOTE
					...
					elseif msg == 0x3 then
						...
						if(songPos==false)then
							...
							if(not isSus)then
								...
								acc = scoreutils:GetAccuracy(scoreutils:GetRating(diff)) or 1
							else
								score=baseScore
								acc = 1
							end
					...
					if BFSongOver and DadSongOver then -- Song Over
						...
						if POI.DadSpot.Owner then
							if not POI.DadSpot.Owner.Character then
								wait(0.5)
							end
							local suc, err = pcall(function()
								POI.DadSpot.Owner.Character.Parent = workspace
								local HRP = POI.DadSpot.Owner.Character:FindFirstChild("HumanoidRootPart")
								if HRP then HRP.Anchored = false end
							end)
						...
						if POI.BFSpot.Owner then
							if not POI.BFSpot.Owner.Character then
								wait(0.5)
							end
							local suc, err = pcall(function()
								POI.BFSpot.Owner.Character.Parent = workspace
								local HRP = POI.BFSpot.Owner.Character:FindFirstChild("HumanoidRootPart")
								if HRP then HRP.Anchored = false end
							end)
							if not suc then warn(err) end
				...
				if POI.BFSpot2.Owner then compRemote:FireClient(POI.BFSpot2.Owner,0x0) end
				if POI.DadSpot2.Owner then compRemote:FireClient(POI.DadSpot2.Owner,0x0) end
				wait(1.5)
				SpotSound:Play()
				SpotVocals:Play()
			end
...
BindFunc.OnInvoke = function(msgType,...)
	...
	if msgType == "GetSpotRemotes" then
		local indicatedSpot = ...
		if indicatedSpot and indicatedSpot:IsDescendantOf(script.Parent) then
